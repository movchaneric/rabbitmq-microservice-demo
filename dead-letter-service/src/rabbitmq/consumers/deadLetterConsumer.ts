...
export function deadLetterConsumer(channel: Channel): void {
  channel.consume(QUEUES.ORDERS_DLX, (msg) => {
    ...
    const deathCount: number = msg.properties.headers?.['x-death']?.[0]?.count ?? 0;

    log(`DLX RECEIVED ⚠ orderId=${order.orderId} attempt=${deathCount}/${MAX_RETRIES}`);

    if (deathCount < MAX_RETRIES) {
      const payload = Buffer.from(JSON.stringify(order));
      channel.publish(EXCHANGES.ORDERS, ROUTING_KEYS.ORDER_PLACED, payload, { persistent: true });
